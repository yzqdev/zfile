<template>
  <div id="List" v-loading="initLoading">
    <el-table
      ...
      :height="
        this.$store.getters.showDocument &&
        this.$store.state.common.config.readme !== null
          ? '50vh'
          : '84vh'
      "
    ...
    >
      ...
      >
        ...
        <template slot-scope="scope">
          <template
            v-if="
              $store.getters.imgMode && scope.row.icon === 'el-icon-my-image'
            "
          >
    ...
    <audio-player
      :file-list="this.$store.getters.filterFileByType('audio')"
...
const VideoPlayer = () =>
  import(/* webpackChunkName: "front-video" */ "./VideoPlayer");
const TextPreview = () =>
  import(/* webpackChunkName: "front-text" */ "./TextPreview");
const AudioPlayer = () =>
  import(/* webpackChunkName: "front-audio" */ "./AudioPlayer");
...
export default {
  ...
  methods: {
    ...
    openBatchCopyLinkDialog() {
      ...
      this.loadLinkData(
        this.$store.getters.tableData[0],
        0,
        this.$store.getters.tableData
      );
    ...
    loadLinkData(item, index, list) {
      ...
      if (item.type === "FILE") {
        ...
        this.$http
          .get("/api/short-link", {
            params: { driveId: this.driveId, path: directlink },
          })
          .then((response) => {
            let link1 = response.data.data;
            let link2 = this.common.removeDuplicateSeparator(
              this.$store.getters.domain +
                "/" +
                this.$store.getters.directLinkPrefix +
                "/" +
                this.driveId +
                "/" +
                encodeURI(item.path) +
    ...
    openFolder(row) {
      ...
      if (row.type === "FILE") {
        ...
        if (row.type === "BACK") {
          ...
          path = this.common.removeDuplicateSeparator(
            row.path + "/" + row.name
          );
        }
    ...
    openVideo() {
      this.currentClickRow.url = this.common.removeDuplicateSeparator(
        this.$store.getters.domain +
          "/" +
          this.$store.getters.directLinkPrefix +
          "/" +
          this.driveId +
          "/" +
          encodeURI(this.currentClickRow.path) +
          "/" +
          encodeURI(this.currentClickRow.name)
      );
        ...
        .catch(() => {
          this.$router.push(
            "/" +
              this.driveId +
              prefixPath +
              path.resolve(this.searchParam.path, "../")
          );
        });
    },
    getPathPwd() {
    ...
    currentClickTypeIndex() {
      return (fileType) => {
        ...
        if (JSON.stringify(currentClickRow) === "{}") {
          ...
          fileType = fileType
            ? fileType
            : this.common.getFileType(currentClickRow.name);
          return this.$store.getters
            .filterFileByType(fileType)
            .findIndex((item) => {
