  ...
  <el-row :gutter="20">
    <el-col :span="12" :offset="6">
      ...
      <el-form ref="form" :rules="rules" :model="form">
        ...
        >
          <el-radio v-model="form.type" @change="changeType" label="Standard"
            >国际版</el-radio
          >
          <el-radio v-model="form.type" @change="changeType" label="China"
            >世纪互联</el-radio
          >
        </el-form-item>
        <el-form-item
        ...
        >
          <template slot="label">
            <span>站点名称</span>
            <span class="zfile-word-aux"
              >（网址上域名后面的 /sites/xxx 或/teams/xxx）</span
            >
          </template>
          <el-input
            v-model="form.siteName"
            @input="submitForm('form')"
            placeholder="请输入站点名称"
...
export default {
  ...
  methods: {
    getDomainPrefix() {
      this.$http
        .post("/sharepoint/getDomainPrefix", this.form)
        .then((response) => {
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.loading = true;
          this.$http
            .post("/sharepoint/getSiteId", this.form)
            .then((response) => {
              if (response.data.code === this.common.responseCode.SUCCESS) {
