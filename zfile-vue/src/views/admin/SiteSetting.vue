    ...
    >
      ...
      >
        ...
        <div class="zfile-word-aux zfile-margin-left-unset">
          此地址用于生成直链及本次存储下载使用，请务必保持和服务端地址一样。
        </div>
      </el-form-item>
  ...
  methods: {
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.loading = true;
          adminConfigApi(this.form).then((response) => {
            this.loading = false;
          ...
          "提示",
          {
            ...
            callback: (action) => {
              if (action === "confirm") {
                ...
                this.$http
                  .post("/admin/config", qs.stringify(this.form))
                  .then((response) => {
                    this.loading = false;
                    if (
                      response.data.code === this.common.responseCode.SUCCESS
                    ) {
                      this.$message({
                        message: "修正成功",
                        type: "success",
