<template>
  <div>
    <el-row :gutter="20" class="zfile-cache-statistics">
      <el-col :span="8">
        <el-card shadow="always">
          ...
          <div class="card-content" v-text="driveCacheInfo.cacheCount"></div>
        </el-card>
      </el-col>
      ...
      <el-col :span="8">
        <el-card shadow="always">
          ...
          <div class="card-content" v-text="driveCacheInfo.hitCount"></div>
        </el-card>
      </el-col>
      <el-col :span="8">
        <el-card shadow="always">
          ...
          <div class="card-content" v-text="driveCacheInfo.missCount"></div>
        </el-card>
      </el-col>
    </el-row>
    ...
    <el-table
      height="50vh"
      :data="
        driveCacheInfo.cacheKeys.filter(
          (data) =>
            !cacheSearch ||
            data.name.toLowerCase().includes(cacheSearch.toLowerCase())
        )
      "
...
export default {
  ...
  methods: {
    refreshCache(row) {
      refreshCacheApi(this.currentCacheManageId, { key: row.name }).then(() => {
        this.$message({
          message: "刷新成功",
          type: "success",
        });
    ...
    loadCacheManageData() {
      cacheInfoApi(this.currentCacheManageId).then((response) => {
    ...
    clearCache() {
      cacheClearApi(this.currentCacheManageId).then(() => {
        this.loadCacheManageData();
        this.$message({
          message: "清理成功",
          type: "success",
