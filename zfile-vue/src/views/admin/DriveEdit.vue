<template>
  <div v-if="supportStrategy">
        ...
        <el-col :span="12">
          ...
          <el-form-item label="是否启用">
            ...
            <el-tooltip placement="right">
              <div slot="content">如不启用，则不在首页展示。</div>
          ...
          >
            <el-col class="el-drive-form-col" :span="22">
              ...
              <div v-else-if="item.key === 'isPrivate'">
                ...
                />
                <el-tooltip placement="right">
                  <div slot="content">私有空间会生成带签名的下载链接</div>
    ...
    <div slot="footer" class="zfile-text-align-center">
      <el-button type="primary" :disabled="loading" @click="submitForm('form')"
        >保 存</el-button
      >
...
import { getSiteApi } from "../../utils/apis";
...
export default {
  ...
  watch: {
    ...
    "driveItem.storageStrategyConfig.siteName"() {
      if (
        !!this.driveItem.storageStrategyConfig.siteName &&
        !!this.driveItem.storageStrategyConfig.accessToken
      ) {
        let fromData = {
        ...
        getSiteApi(FormData).then((response) => {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        "storageStrategyConfig.domain": [
          {
            ...
            validator: (rule, value, callback) => {
              ...
              if (!domainCheck.test(value)) {
                callback(
                  new Error("请输入正确的域名，需以 http:// 或 https:// 开头")
                );
        ...
        "storageStrategyConfig.username": [
          {
            validator: (rule, value, callback) => {
              if (
                (this.driveItem.type === "upyun" ||
                  this.driveItem.type === "ufile") &&
                (value === undefined || value === "")
              ) {
        ...
        "storageStrategyConfig.endPoint": [
          { required: true, message: "区域不能为空" },
        ],
        "storageStrategyConfig.accessKey": [
          { required: true, message: "AccessKey 不能为空" },
        ],
        "storageStrategyConfig.filePath": [
          { required: true, message: "文件路径不能为空" },
        ],
        "storageStrategyConfig.secretKey": [
          { required: true, message: "SecretKey 不能为空" },
        ],
        "storageStrategyConfig.bucketName": [
          { required: true, message: "此项不能为空" },
        ],
        "storageStrategyConfig.host": [
          { required: true, message: "域名或 IP 不能为空" },
        ],
        "storageStrategyConfig.port": [
          { required: true, message: "端口不能为空" },
        ],
        "storageStrategyConfig.accessToken": [
          { required: true, message: "访问令牌不能为空" },
        ],
        "storageStrategyConfig.refreshToken": [
          { required: true, message: "刷新令牌不能为空" },
        ],
        "storageStrategyConfig.secretId": [
          { required: true, message: "SecretId 不能为空" },
        ],
        "storageStrategyConfig.siteId": [
          { required: true, message: "SiteId 不能为空" },
        ],
      },
    };
  },
  methods: {
    ...
    loadStrategyForm(val) {
      this.$http
        .get("/admin/strategy-form", { params: { storageType: val } })
        .then((response) => {
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.loading = true;
          this.$http
            .post("/admin/drive", this.driveItem)
            .then((response) => {
